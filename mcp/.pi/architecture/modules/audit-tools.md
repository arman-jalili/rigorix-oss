...
- AuditEnvelope HMAC integrity is **verified on read** by the store: when an HMAC key is configured, `read_audit`, `list_audits`, and `audit_summary` recompute each record's HMAC and refuse a mismatch (ADR-016 verify-on-read); the gateway never serves tampered evidence
...
HMAC key (`with_hmac_key`) and `read_audit`, `list_audits`, and `audit_summary`
recompute the envelope's HMAC, rejecting a mismatch with `AuditError::Internal`
— a tampered in-memory record is never returned (a tampered record also fails
the whole listing/summary, fail closed). With no key configured (tests),
