...
flowchart LR
    ...
    FACADE -->|"run_from_template(steps, repo, author)"| EXEC_ENG
...
classDiagram
    class EngineFacade {
        <<interface>>
        +execute(plan: PlanTemplate, repository?: string, author?: string) Result~ExecutionResult~
...
sequenceDiagram
    ...
    Facade->>Engine: run_from_template(steps, repo, author)
...
- `repository` and `author` are auto-derived from git (`git remote get-url origin` → "owner/repo", `git config user.email`) when not provided explicitly by the caller
...
impl ExecuteHandler {
    ...
    pub async fn handle(&self, input: ExecuteInput) -> Result<ToolCallResult, HandlerError> {
        ...
        // 2. Execute with timeout, passing repository and author for audit
        let result = tokio::time::timeout(
            self.timeout_duration,
            self.engine.execute(plan, input.repository, input.author),
        )
        .await
