...
guardian_issue:
  ...
  file_changes:
    - "create: .pi/scripts/ci/check_audit-tools_contracts.sh"
    - "create: .pi/scripts/ci/check_audit-tools_coverage.sh"
    - "modify: .pi/scripts/ci/run_hardening_stages.sh"
---

# Proofing & CI Enforcement: audit-tools
...
|--------|---------|----------|
| check_audit-tools_contracts.sh | Validate contract implementation | .pi/scripts/ci/ |
| check_audit-tools_coverage.sh | Enforce coverage thresholds | .pi/scripts/ci/ |
| stage_audit-tools_proofing.sh | CI stage wrapper | .pi/scripts/ci/ |
...
```bash
run_stage "11" "audit-tools_proofing" \
    "${SCRIPTS_DIR}/stage_audit-tools_proofing.sh" \
    "always"
```
