	...
	async startEpic(
		...
		if (repoTool === "glab" ? commandExists("glab") : commandExists("gh")) {
			...
			if (authCheck.exitCode === 0) {
				// Verify project-level access, not just global auth
				const projectAccessCheck = repository ? runScript(
					this.cwd,
					repoTool === "glab"
				...
				if (projectAccessCheck.exitCode === 0 && projectAccessCheck.stdout.trim().length > 0) {
