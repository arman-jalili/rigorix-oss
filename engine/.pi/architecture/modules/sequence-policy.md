...
- **Integrity (ADR-016 Phase A).** The adapter **verifies each envelope's HMAC (when `RIGORIX_HMAC_KEY` is available) and the local per-producer chain on read**; a tampered/gapped store fails closed. Because local history is `local_unanchored`, a **deny-class** cross-run rule that would fire on it refuses by default; `RIGORIX_HISTORY_POLICY=allow_unanchored` records an explicit best-effort opt-in (`SequencePolicyServiceImpl::with_unanchored_history_allowed`), and the opt-in is written into each envelope. Legacy (unchained) envelopes are grandfathered. Tail deletion is not detectable locally — the anchored mode below closes it against the external anchor. A refusal is **structured**: `SequencePolicyError::HistoryUnanchored` → `OrchestratorError::HistoryUnanchoredRefused` → native `policy_violation` (`-32010`) carrying `data.rule_id` / `data.step` and `data.details.reason = "history_unanchored"` — never an opaque `-32603 internal_error`.
- **Anchored mode (ADR-016 Phase C, #899).** With `RIGORIX_ANCHOR_URL` + `RIGORIX_ANCHOR_PUBLIC_KEY` set, `SequencePolicySetup::from_env` swaps in **`AnchoredHistoryAdapter`** over the shared `AnchorRuntime`: it fetches the anchor-signed history projection (`HistorySlice`) and verifies the **Ed25519 signature (`verify_slice`) before returning any action** — the matcher never sees unverified evidence. An unreachable anchor or a forged/tampered slice is an `Err`, which the service **fails closed** on for consequential (deny-class / history-dependent) runs; `is_anchored()` is `true` because every read is signature-gated, so the unanchored refusal above does not apply. Envelopes are tagged `history_integrity = anchored` and carry the verified `anchor_head`. With neither env var set the Phase A local adapter is used, unchanged.
