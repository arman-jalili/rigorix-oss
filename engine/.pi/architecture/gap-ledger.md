...
**GAP-A-30 Phase A resolution (#898).** The OSS/local half of ADR-016 landed:
`AuditEnvelope` carries `producer_id` / `sequence` / `prev_hash` /
`history_integrity` / `history_policy` (additive, signed);
`AuditServiceImpl::build_and_send` resolves and signs the chain link;
`application/chain.rs` verifies continuity (interior deletion / reorder /
insertion / `prev_hash` mismatch) and the cross-run guard
(`EnvelopeHistoryAdapter`) verifies each envelope's HMAC and the chain on read
