#!/usr/bin/env bash
# ============================================================================
# check_cancellation_coverage.sh
#
# Enforces minimum code coverage thresholds for the cancellation module.
# Uses cargo-tarpaulin or llvm-cov to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_cancellation_coverage.sh [--help]
#
...
if command -v cargo-tarpaulin &>/dev/null; then
    ...
    # Fallback: count test functions in cancellation module
    TEST_COUNT=$(grep -rn 'mod tests' "$PROJECT_ROOT/src/cancellation/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge 2 ]; then
        log_pass "Test modules: $TEST_COUNT test modules in cancellation (minimum 2 required)"
    else
        log_fail "Only $TEST_COUNT test modules found in cancellation (minimum 2 required)"
    fi
fi
