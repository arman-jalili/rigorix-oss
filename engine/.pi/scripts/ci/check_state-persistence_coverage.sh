#!/usr/bin/env bash
# ============================================================================
# check_state-persistence_coverage.sh
#
# Enforces minimum code coverage thresholds for the state-persistence module.
# Uses cargo-llvm-cov or tarpaulin to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_state-persistence_coverage.sh [--help]
#
...
echo ""
echo "═══ State-Persistence Coverage Threshold Check ═══"
echo ""
...
if [[ "${RIGORIX_COVERAGE:-}" == "1" ]] && command -v cargo-llvm-cov &>/dev/null; then
    ...
    COVERAGE_OUTPUT=$(cargo tarpaulin --out Xml --output-dir ../coverage --exclude-files "src/state_persistence/interfaces/*" 2>&1 || true)
    ...
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]\|^\s*#\[test\]' "$PROJECT_ROOT/src/state_persistence/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge "$MIN_TEST_COUNT" ]; then
        log_pass "Test count: $TEST_COUNT tests in state_persistence module (minimum $MIN_TEST_COUNT required)"
    else
        log_fail "Only $TEST_COUNT tests found in state_persistence module (minimum $MIN_TEST_COUNT required)"
    fi
fi
