#!/usr/bin/env bash
# ============================================================================
# stage_cancellation_proofing.sh
#
# CI stage wrapper that runs all cancellation proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_cancellation_proofing.sh [--help]
#
...
if bash "${SCRIPT_DIR}/check_cancellation_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_cancellation_coverage.sh" 2>&1; then
...
for script in "$SCRIPT_DIR"/check_cancellation_*.sh; do
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Cancellation proofing stage FAILED."
    exit 1
fi

echo "Cancellation proofing stage PASSED."
exit 0
