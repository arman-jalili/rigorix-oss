#!/usr/bin/env bash
# ============================================================================
# stage_audit_proofing.sh
#
# CI stage wrapper that runs all audit proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_audit_proofing.sh [--help]
#
...
if bash "${SCRIPT_DIR}/check_audit_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_audit_coverage.sh" 2>&1; then
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Audit proofing stage FAILED."
    exit 1
fi

echo "Audit proofing stage PASSED."
exit 0
