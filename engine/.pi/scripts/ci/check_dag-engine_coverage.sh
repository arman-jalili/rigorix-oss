#!/usr/bin/env bash
# ============================================================================
# check_dag-engine_coverage.sh
#
# Enforces minimum code coverage thresholds for the dag-engine module.
# Uses cargo-tarpaulin or llvm-cov to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_dag-engine_coverage.sh [--help]
#
...
echo ""
echo "═══ DAG-Engine Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-tarpaulin &>/dev/null; then
    ...
    COVERAGE_OUTPUT=$(cargo tarpaulin --out Xml --output-dir ../coverage --exclude-files "src/dag_engine/interfaces/*" 2>&1 || true)
    ...
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]\|^\s*#\[test\]' "$PROJECT_ROOT/src/dag_engine/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge "$MIN_TEST_COUNT" ]; then
        log_pass "Test count: $TEST_COUNT tests in dag_engine module (minimum $MIN_TEST_COUNT required)"
    else
        log_fail "Only $TEST_COUNT tests found in dag_engine module (minimum $MIN_TEST_COUNT required)"
    fi
fi
