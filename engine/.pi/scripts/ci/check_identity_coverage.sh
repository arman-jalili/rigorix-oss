#!/usr/bin/env bash
# ============================================================================
# check_identity_coverage.sh
#
# Enforces minimum code coverage thresholds for the identity module.
# Uses cargo-llvm-cov or tarpaulin to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_identity_coverage.sh [--help]
#
...
echo ""
echo "═══ Identity Coverage Threshold Check ═══"
echo ""
...
if [[ "${RIGORIX_COVERAGE:-}" == "1" ]] && command -v cargo-llvm-cov &>/dev/null; then
    ...
    # Fallback: count test functions in identity module
    TEST_COUNT=$(grep -rE '^\s*#\[(tokio::)?test\]' "$PROJECT_ROOT/src/identity/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge 15 ]; then
        log_pass "Test count: $TEST_COUNT tests in identity module (minimum 15 required)"
    else
        log_fail "Only $TEST_COUNT tests found in identity module (minimum 15 required)"
    fi
fi
