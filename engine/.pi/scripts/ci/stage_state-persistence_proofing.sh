#!/usr/bin/env bash
# ============================================================================
# stage_state-persistence_proofing.sh
#
# CI stage wrapper that runs all state-persistence proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_state-persistence_proofing.sh [--help]
#
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║    State-Persistence Proofing Stage              ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
if bash "${SCRIPT_DIR}/check_state-persistence_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_state-persistence_coverage.sh" 2>&1; then
...
for script in "$SCRIPT_DIR"/check_state-persistence_*.sh; do
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "State-persistence proofing stage FAILED."
    exit 1
fi

echo "State-persistence proofing stage PASSED."
exit 0
