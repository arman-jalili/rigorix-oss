#!/usr/bin/env bash
# ============================================================================
# check_event-system_coverage.sh
#
# Enforces minimum code coverage thresholds for the event-system module.
# Uses cargo-llvm-cov or tarpaulin to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_event-system_coverage.sh [--help]
#
...
echo ""
echo "═══ Event-System Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-llvm-cov &>/dev/null; then
    ...
    COVERAGE_OUTPUT=$(cargo tarpaulin --out Xml --output-dir ../coverage --exclude-files "src/event_system/interfaces/*" 2>&1 || true)
    ...
    # Count test functions in event_system module
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]\|^\s*#\[test\]' "$PROJECT_ROOT/src/event_system/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge "$MIN_TEST_COUNT" ]; then
        log_pass "Test count: $TEST_COUNT tests in event_system module (minimum $MIN_TEST_COUNT required)"
    else
        log_fail "Only $TEST_COUNT tests found in event_system module (minimum $MIN_TEST_COUNT required)"
    fi
fi
