#!/usr/bin/env bash
# ============================================================================
# stage_enforcement_proofing.sh
#
# CI stage wrapper that runs all enforcement proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_enforcement_proofing.sh [--help]
#
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║       Enforcement Proofing Stage                ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
if bash "${SCRIPT_DIR}/check_enforcement_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_enforcement_coverage.sh" 2>&1; then
...
for script in "$SCRIPT_DIR"/check_enforcement_*.sh; do
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Enforcement proofing stage FAILED."
    exit 1
fi

echo "Enforcement proofing stage PASSED."
exit 0
