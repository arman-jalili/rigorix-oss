...
echo ""
echo "═══ Configuration Coverage Threshold Check ═══"
echo ""
...
if [[ "${RIGORIX_COVERAGE:-}" == "1" ]] && command -v cargo-llvm-cov &>/dev/null; then
    ...
    # Fallback: count test functions vs total functions
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]' "$PROJECT_ROOT/src/configuration/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge 10 ]; then
        log_pass "Test count: $TEST_COUNT tests (minimum 10 required)"
    else
        log_fail "Only $TEST_COUNT tests found (minimum 10 required)"
    fi
fi
