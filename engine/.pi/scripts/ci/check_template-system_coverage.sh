#!/usr/bin/env bash
# ============================================================================
# check_template-system_coverage.sh
#
# Enforces minimum code coverage thresholds for the template-system module.
# Uses cargo-tarpaulin or llvm-cov to measure line coverage. Falls back to
# counting tests if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_template-system_coverage.sh [--help]
#
...
echo ""
echo "═══ Template-System Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-tarpaulin &>/dev/null; then
    ...
    # Fallback: count test functions in template-system module
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]' "$PROJECT_ROOT/src/templates/" 2>/dev/null | wc -l | tr -d ' ')
    TOTAL_TESTS=$((TEST_COUNT))

    if [ "$TOTAL_TESTS" -ge 15 ]; then
        log_pass "Test count: $TOTAL_TESTS tests (minimum 15 required)"
    else
        log_fail "Only $TOTAL_TESTS tests found (minimum 15 required)"
    fi
fi
