#!/usr/bin/env bash
# ============================================================================
# check_enforcement_coverage.sh
#
# Enforces minimum code coverage thresholds for the enforcement module.
# Uses cargo-tarpaulin or llvm-cov to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_enforcement_coverage.sh [--help]
#
...
echo ""
echo "═══ Enforcement Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-tarpaulin &>/dev/null; then
    ...
    # Fallback: count test functions in enforcement module
    TEST_COUNT=$(grep -rn 'mod tests' "$PROJECT_ROOT/src/enforcement/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge 3 ]; then
        log_pass "Test modules: $TEST_COUNT test modules in enforcement (minimum 3 required)"
    else
        log_fail "Only $TEST_COUNT test modules found in enforcement (minimum 3 required)"
    fi
fi
