#!/usr/bin/env bash
# ============================================================================
# stage_scored-evaluation_proofing.sh
#
# CI stage wrapper that runs all scored-evaluation proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_scored-evaluation_proofing.sh [--help]
#
...
echo ""
echo "╔══════════════════════════════════════════════════════╗"
echo "║       Scored Evaluation Proofing Stage               ║"
echo "╚══════════════════════════════════════════════════════╝"
echo ""
...
if bash "${SCRIPT_DIR}/check_scored-evaluation_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_scored-evaluation_coverage.sh" 2>&1; then
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Scored Evaluation proofing stage FAILED."
    exit 1
fi

echo "Scored Evaluation proofing stage PASSED."
exit 0
