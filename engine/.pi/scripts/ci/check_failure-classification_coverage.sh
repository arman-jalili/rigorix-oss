#!/usr/bin/env bash
# ============================================================================
# check_failure-classification_coverage.sh
#
# Enforces minimum code coverage thresholds for the failure-classification
# module. Uses cargo-tarpaulin or llvm-cov to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_failure-classification_coverage.sh [--help]
#
...
echo ""
echo "═══ Failure Classification Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-tarpaulin &>/dev/null; then
    ...
    # Fallback: count test functions in failure-classification module
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]' "$PROJECT_ROOT/src/failure_classification/" 2>/dev/null | wc -l | tr -d ' ')
    TOTAL_TESTS=$((TEST_COUNT + $(grep -r '^\s*#\[test\]' "$PROJECT_ROOT/src/failure_classification/" 2>/dev/null | wc -l | tr -d ' ')))

    if [ "$TOTAL_TESTS" -ge 30 ]; then
        log_pass "Test count: $TOTAL_TESTS tests in failure-classification module (minimum 30 required)"
    else
        log_fail "Only $TOTAL_TESTS tests found in failure-classification module (minimum 30 required)"
    fi
fi
