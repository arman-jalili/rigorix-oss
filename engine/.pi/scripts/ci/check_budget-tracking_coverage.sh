#!/usr/bin/env bash
# ============================================================================
# check_budget-tracking_coverage.sh
#
# Enforces minimum code coverage thresholds for the budget-tracking module.
# Uses cargo-llvm-cov or tarpaulin to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_budget-tracking_coverage.sh [--help]
#
...
echo ""
echo "═══ Budget-Tracking Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-llvm-cov &>/dev/null; then
    ...
    # Fallback: count test functions in budget_tracking module
    TEST_COUNT=$(grep -rn 'fn test_' "$PROJECT_ROOT/src/budget_tracking/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge 20 ]; then
        log_pass "Test functions: $TEST_COUNT in budget-tracking (minimum 20 required)"
    else
        log_fail "Only $TEST_COUNT test functions found in budget-tracking (minimum 20 required)"
    fi
fi
