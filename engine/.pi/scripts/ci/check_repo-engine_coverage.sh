#!/usr/bin/env bash
# ============================================================================
# check_repo-engine_coverage.sh
#
# Enforces minimum code coverage thresholds for the repo-engine module.
# Uses cargo-llvm-cov or tarpaulin to measure line coverage. Falls back to
# counting tests if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_repo-engine_coverage.sh [--help]
#
...
echo ""
echo "═══ Repo-Engine Coverage Threshold Check ═══"
echo ""
...
if [[ "${RIGORIX_COVERAGE:-}" == "1" ]] && command -v cargo-llvm-cov &>/dev/null; then
    ...
    # Fallback: count test functions in repo-engine module
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]' "$PROJECT_ROOT/src/repo_engine/" 2>/dev/null | wc -l | tr -d ' ')
    TOTAL_TESTS=$((TEST_COUNT))

    if [ "$TOTAL_TESTS" -ge 20 ]; then
        log_pass "Test count: $TOTAL_TESTS tests (minimum 20 required)"
    else
        log_fail "Only $TOTAL_TESTS tests found (minimum 20 required)"
    fi
fi
