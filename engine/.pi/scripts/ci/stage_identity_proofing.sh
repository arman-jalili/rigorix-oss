#!/usr/bin/env bash
# ============================================================================
# stage_identity_proofing.sh
#
# CI stage wrapper that runs all identity proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_identity_proofing.sh [--help]
#
...
if bash "${SCRIPT_DIR}/check_identity_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_identity_coverage.sh" 2>&1; then
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Identity proofing stage FAILED."
    exit 1
fi

echo "Identity proofing stage PASSED."
exit 0
