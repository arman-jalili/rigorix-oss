#!/usr/bin/env bash
# ============================================================================
# check_audit_coverage.sh
#
# Enforces minimum code coverage thresholds for the audit module.
# Uses cargo-llvm-cov or tarpaulin to measure line coverage.
# Falls back to test count if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_audit_coverage.sh [--help]
#
...
echo ""
echo "═══ Audit Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-llvm-cov &>/dev/null; then
    ...
    # Fallback: count test functions in audit module
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]' "$PROJECT_ROOT/src/audit/" 2>/dev/null | wc -l | tr -d ' ')
    if [ "$TEST_COUNT" -ge 15 ]; then
        log_pass "Test count: $TEST_COUNT tests in audit module (minimum 15 required)"
    else
        log_fail "Only $TEST_COUNT tests found in audit module (minimum 15 required)"
    fi
fi
