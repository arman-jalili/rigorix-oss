#!/usr/bin/env bash
# ============================================================================
# check_tool-system_coverage.sh
#
# Enforces minimum code coverage thresholds for the tool-system module.
# Uses cargo-tarpaulin or llvm-cov to measure line coverage. Falls back to
# counting tests if no coverage tool is available.
#
# Usage: bash .pi/scripts/ci/check_tool-system_coverage.sh [--help]
#
...
echo ""
echo "═══ Tool-System Coverage Threshold Check ═══"
echo ""
...
if command -v cargo-tarpaulin &>/dev/null; then
    ...
    # Fallback: count test functions in tool-system module
    TEST_COUNT=$(grep -r '^\s*#\[tokio::test\]' "$PROJECT_ROOT/src/tools/" 2>/dev/null | wc -l | tr -d ' ')
