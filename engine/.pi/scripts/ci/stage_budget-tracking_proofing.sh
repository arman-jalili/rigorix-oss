#!/usr/bin/env bash
# ============================================================================
# stage_budget-tracking_proofing.sh
#
# CI stage wrapper that runs all budget-tracking proofing checks:
#   1. Contract implementation check — all interfaces have implementations
#   2. Coverage threshold check — meets minimum coverage
#
# Usage: bash .pi/scripts/ci/stage_budget-tracking_proofing.sh [--help]
#
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║    Budget-Tracking Proofing Stage            ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
if bash "${SCRIPT_DIR}/check_budget-tracking_contracts.sh" 2>&1; then
...
if bash "${SCRIPT_DIR}/check_budget-tracking_coverage.sh" 2>&1; then
...
for script in "$SCRIPT_DIR"/check_budget-tracking_*.sh; do
...
if [ ${#ERRORS[@]} -gt 0 ]; then
    ...
    echo ""
    echo "Budget-tracking proofing stage FAILED."
    exit 1
fi

echo "Budget-tracking proofing stage PASSED."
exit 0
