...
guardian_issue:
  id: "ISSUE-IDENTITY-3"
  epic: "TBD"
  component: "IdentityAttestationService"
  module: "identity"
  status: planned
  priority: high
  dependencies:
  ...
  in_scope:
    ...
    - "ApproveInput.approver_id populated from claim; recorded in ApprovalRecord"
  ...
  implementation_notes: |
    ...
    Component focus: IdentityAttestationService.
...
# ISSUE-IDENTITY-3: Implement IdentityAttestationService — identity

## Intent

Implement **IdentityAttestationService** for the `identity` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 3 | `extract_claims` decodes standard JWT claims (sub, iss, exp, roles) | unit test |
| 4 | attest with unreachable IdP → `IdentitySource::Unverified`, explicit marker, no error | integration test |
| 5 | verify against mock JWKS: valid → Verified; tampered → Unverified | integration test |
| 7 | ApproveInput.approver_id populated from claim; recorded in ApprovalRecord | integration test |
...
> 2. Identify which rows are your responsibility for **IdentityAttestationService**
