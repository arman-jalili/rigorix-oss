---
guardian_issue:
  id: "ISSUE-SCORED-EVALUATION-10"
  epic: "TBD"
  component: "ScoredEvaluationService"
  ...
  in_scope:
    - "Orchestrates: validate → emit Started → backend → emit Completed/Failed → persist"
  ...
  implementation_notes: |
    ...
    Component focus: ScoredEvaluationService.
...
# ISSUE-SCORED-EVALUATION-10: Implement ScoredEvaluationService — scored-evaluation

## Intent

Implement **ScoredEvaluationService** for the `scored-evaluation` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 9 | Orchestrates: validate → emit Started → backend → emit Completed/Failed → persist | integration test |
...
> 2. Identify which rows are your responsibility for **ScoredEvaluationService**
