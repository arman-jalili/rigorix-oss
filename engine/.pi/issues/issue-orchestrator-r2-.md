---
guardian_issue:
  id: "ISSUE-SEQUENCE-POLICY-6"
  epic: "TBD"
  component: "Orchestrator (R2)"
  ...
  in_scope:
    - "Runbook containing remove-then-add: later step is built `requires_approval=true`; run pauses; approve → executes; reject → skipped"
    - "`deny` rule: later step fails with `SequencePolicyDenied`, tool never called (spy tool asserts)"
  ...
  implementation_notes: |
    ...
    Component focus: Orchestrator (R2).
...
# ISSUE-SEQUENCE-POLICY-6: Implement Orchestrator (R2) — sequence-policy

## Intent

Implement **Orchestrator (R2)** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 6 | Runbook containing remove-then-add: later step is built `requires_approval=true`; run pauses; approve → executes; reject → skipped | integration test |
| 7 | `deny` rule: later step fails with `SequencePolicyDenied`, tool never called (spy tool asserts) | integration test |
...
> 2. Identify which rows are your responsibility for **Orchestrator (R2)**
