...
guardian_issue:
  ...
  in_scope:
    - "`rigorix_validate_plan` on a plan with a matched sequence returns a structured finding before run"
...
# ISSUE-SEQUENCE-POLICY-7: Implement MCP surface — sequence-policy

## Intent

Implement **MCP surface** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 8 | `rigorix_validate_plan` on a plan with a matched sequence returns a structured finding before run | integration test |
...
> 2. Identify which rows are your responsibility for **MCP surface**
