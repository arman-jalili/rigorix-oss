...
guardian_issue:
  ...
  in_scope:
    - "approve with decision_context → envelope contains context ref + summary; full payload opt-in"
    - "api_key inside decision_context → redacted in summary (SpanPrivacy reuse)"
  ...
  implementation_notes: |
    ...
    Component focus: DecisionContext.
...
# ISSUE-APPROVAL-4: Implement DecisionContext — approval

## Intent

Implement **DecisionContext** for the `approval` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 13 | approve with decision_context → envelope contains context ref + summary; full payload opt-in | integration test |
| 14 | api_key inside decision_context → redacted in summary (SpanPrivacy reuse) | unit test |
...
> 2. Identify which rows are your responsibility for **DecisionContext**
