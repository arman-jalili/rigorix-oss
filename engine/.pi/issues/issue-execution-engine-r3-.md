---
guardian_issue:
  id: "ISSUE-SEQUENCE-POLICY-8"
  epic: "TBD"
  component: "Execution Engine (R3)"
  ...
  in_scope:
    - "Dynamic plan completes step A, then proposes B (would complete) → B promoted/denied per rule"
  ...
  implementation_notes: |
    ...
    Component focus: Execution Engine (R3).
...
# ISSUE-SEQUENCE-POLICY-8: Implement Execution Engine (R3) — sequence-policy

## Intent

Implement **Execution Engine (R3)** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 9 | Dynamic plan completes step A, then proposes B (would complete) → B promoted/denied per rule | integration test |
...
> 2. Identify which rows are your responsibility for **Execution Engine (R3)**
