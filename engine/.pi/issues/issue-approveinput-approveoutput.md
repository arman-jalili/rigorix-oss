...
guardian_issue:
  id: "ISSUE-APPROVAL-7"
  epic: "TBD"
  component: "ApproveInput / ApproveOutput"
  module: "approval"
  status: planned
  priority: high
  dependencies:
  ...
  in_scope:
    - "`from_node` on sealed node produces canonical intent; `canonical_bytes` deterministic (property test)"
    - "Same tool+intent+scope → same hash; any byte change → different hash"
    - "Serde round-trip preserves all fields; status transitions valid (Pending → Consumed/Expired/Superseded)"
  ...
  implementation_notes: |
    ...
    Component focus: ApproveInput / ApproveOutput.
...
# ISSUE-APPROVAL-7: Implement ApproveInput / ApproveOutput — approval

## Intent

Implement **ApproveInput / ApproveOutput** for the `approval` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 1 | `from_node` on sealed node produces canonical intent; `canonical_bytes` deterministic (property test) | unit test |
| 2 | Same tool+intent+scope → same hash; any byte change → different hash | unit test |
| 3 | Serde round-trip preserves all fields; status transitions valid (Pending → Consumed/Expired/Superseded) | unit test |
...
> 2. Identify which rows are your responsibility for **ApproveInput / ApproveOutput**
