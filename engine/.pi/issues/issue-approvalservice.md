...
guardian_issue:
  ...
  in_scope:
    - "approve → dispatch identical intent → executes; envelope contains signed `ApprovalRecorded`"
    - "approve → intent mutated (simulated upstream change / tampered state) → **HALT before dispatch**, `IntentMismatch`, tool never called (spy tool asserts)"
    - "Cross-process: pause in A, tamper persisted intent in state file, resume in B → halted, re-approval required"
    - "Retry path: failed approved node retries with same intent → per-attempt verify passes; a failed attempt does NOT consume; consume happens once on terminal outcome"
    - "Replay: consumed approval replayed against same step → rejected (single-use + nonce)"
    - "TTL: approval expires between approve and dispatch → `Invalid(Expired)`, no dispatch"
  ...
  acceptance_criteria:
    ...
    - "Replay: consumed approval replayed against same step → rejected (single-use + nonce)"
    - "TTL: approval expires between approve and dispatch → `Invalid(Expired)`, no dispatch"
    - "Migration: persisted `approved` without records → invalidated on hydrate, re-approval required"
  ...
  implementation_notes: |
    ...
    Component focus: ApprovalService.
...
# ISSUE-APPROVAL-6: Implement ApprovalService — approval

## Intent

Implement **ApprovalService** for the `approval` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 4 | approve → dispatch identical intent → executes; envelope contains signed `ApprovalRecorded` | integration test |
| 5 | approve → intent mutated (simulated upstream change / tampered state) → **HALT before dispatch**, `IntentMismatch`, tool never called (spy tool asserts) | integration test |
| 6 | Cross-process: pause in A, tamper persisted intent in state file, resume in B → halted, re-approval required | integration test |
| 7 | Retry path: failed approved node retries with same intent → per-attempt verify passes; a failed attempt does NOT consume; consume happens once on terminal outcome | integration test |
| 8 | Replay: consumed approval replayed against same step → rejected (single-use + nonce) | integration test |
| 9 | TTL: approval expires between approve and dispatch → `Invalid(Expired)`, no dispatch | unit test |
| 10 | Migration: persisted `approved` without records → invalidated on hydrate, re-approval required | integration test |
...
> 2. Identify which rows are your responsibility for **ApprovalService**
