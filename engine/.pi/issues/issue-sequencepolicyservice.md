---
guardian_issue:
  id: "ISSUE-SEQUENCE-POLICY-2"
  epic: "TBD"
  component: "SequencePolicyService"
  ...
  in_scope:
    - "`evaluate_plan` finds remove-then-add pair in a runbook; returns later step id"
  ...
  implementation_notes: |
    ...
    Component focus: SequencePolicyService.
...
# ISSUE-SEQUENCE-POLICY-2: Implement SequencePolicyService — sequence-policy

## Intent

Implement **SequencePolicyService** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 5 | `evaluate_plan` finds remove-then-add pair in a runbook; returns later step id | unit test |
...
> 2. Identify which rows are your responsibility for **SequencePolicyService**
