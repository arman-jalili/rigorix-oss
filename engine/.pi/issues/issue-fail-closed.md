...
guardian_issue:
  ...
  in_scope:
    - "Corrupt rule config → plan refused, no steps execute"
...
# ISSUE-SEQUENCE-POLICY-9: Implement Fail-closed — sequence-policy

## Intent

Implement **Fail-closed** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 10 | Corrupt rule config → plan refused, no steps execute | integration test |
...
> 2. Identify which rows are your responsibility for **Fail-closed**
