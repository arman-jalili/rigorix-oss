...
guardian_issue:
  ...
  in_scope:
    - "TOML parse → rule with ordered predicates + action; serde round-trip preserves all fields"
...
# ISSUE-SEQUENCE-POLICY-1: Implement SequenceRule — sequence-policy

## Intent

Implement **SequenceRule** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 1 | TOML parse → rule with ordered predicates + action; serde round-trip preserves all fields | unit test |
...
> 2. Identify which rows are your responsibility for **SequenceRule**
