...
guardian_issue:
  ...
  in_scope:
    - "Adjacent pair match; windowed match (gap ≤ window); out-of-window does not match"
    - "Determinism property test: same ordered plan + rules → same match set"
...
# ISSUE-SEQUENCE-POLICY-5: Implement Matcher — sequence-policy

## Intent

Implement **Matcher** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 3 | Adjacent pair match; windowed match (gap ≤ window); out-of-window does not match | unit test |
| 4 | Determinism property test: same ordered plan + rules → same match set | unit test (property) |
...
> 2. Identify which rows are your responsibility for **Matcher**
