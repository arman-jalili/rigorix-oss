...
guardian_issue:
  ...
  in_scope:
    - "Matched rule + promotion recorded in envelope events; decision summaries redact parameter values by default (SpanPrivacy pattern)"
...
# ISSUE-SEQUENCE-POLICY-11: Implement Audit (R6) — sequence-policy

## Intent

Implement **Audit (R6)** for the `sequence-policy` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 12 | Matched rule + promotion recorded in envelope events; decision summaries redact parameter values by default (SpanPrivacy pattern) | integration test |
...
> 2. Identify which rows are your responsibility for **Audit (R6)**
