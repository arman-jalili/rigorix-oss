---
guardian_issue:
  id: "ISSUE-SCORED-EVALUATION-11"
  epic: "TBD"
  component: "EvaluateInput / EvaluateOutput"
  ...
  in_scope:
    - "Serde round-trip: serialize and deserialize preserves all fields"
    - "Inline and Reference sources serialize correctly with tagged enum"
    - "`passed` flag computed correctly from per-dimension `passed`"
    - "`passed` correctly calculated from score vs max"
  ...
  implementation_notes: |
    ...
    Component focus: EvaluateInput / EvaluateOutput.
...
# ISSUE-SCORED-EVALUATION-11: Implement EvaluateInput / EvaluateOutput — scored-evaluation

## Intent

Implement **EvaluateInput / EvaluateOutput** for the `scored-evaluation` module.
...
| # | Criterion | Verify In |
|---|-----------|-----------|
| 1 | Serde round-trip: serialize and deserialize preserves all fields | unit test |
| 2 | Inline and Reference sources serialize correctly with tagged enum | unit test |
| 3 | `passed` flag computed correctly from per-dimension `passed` | unit test |
| 4 | `passed` correctly calculated from score vs max | unit test |
...
> 2. Identify which rows are your responsibility for **EvaluateInput / EvaluateOutput**
