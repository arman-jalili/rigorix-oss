...
created: 2026-06-13
business_context: "# Rigorix Architecture Specification **Version:** 3.2.0 **Status:** Architecture Blueprint - Implementation Fidelity Tracked Separately **Last Updated:** 2026-05-24 **Last Code-Synced:** 2026-05-26 (ARCH-GAP-022 closed) **Architecture Blueprint Quality:** 9.5/10 **Implementation Gap Ledger:** [`docs/ARCHITECTURE_GAPS.md`](ARCHITECTURE_GAPS.md) **Backend Gap Ledger:** [`docs/BACKEND_GAPS.md`](BACKEND_GAPS.md) **Audience:** Human developers AND AI agents --- ## Quick Reference ### Architecture Blueprint Quality | Aspect | Score | Status | |--------|-------|--------| | Conceptual design | 9.5/10 | Deterministic task graph compiler, not an agent swarm | | Scope realism | 9.5/10 | Bounded autonomy, planning-only LLM, explicit execution profiles | | Implementability | 9.0/10 | Clear module contracts and Rust-first operational constraints | | Verification model | 9.0/10 | Architecture contracts, CI gates, replay requirements, gap ledger | | Implementation fidelity | Tracked separately | See [`ARCHITECTURE_GAPS.md`](ARCHITECTURE_GAPS.md) | | **Blueprint overall** | **9.5/10** | Strong engineering specification; production status depends on gap closure | ### Blueprint Operating Rules This document is the target architecture, not a release-status report. It defines the system Rigorix must implement. Code reality, known deviations, and remediation work are tracked in [`docs/ARCHITECTURE_GAPS.md`](ARCHITECTURE_GAPS.md). Architecture claims use these meanings: | Claim Type | Meaning | Required Evidence | |------------|---------|-------------------| | **Blueprint requirement** | A design constraint that implementation must satisfy | Section in this document plus acceptance criteria | | **Implemented invariant** | A requirement already proven by code/tests | Test, contract, or CI gate reference | | **Known gap** | A documented mismatch between blueprint and code | Entry in `ARCHITECTURE_GAPS.md` | | **Enterprise gate** | A required quality/sanity check before release | CI command, contract test, or manual review checklist | No future architecture change may be merged without: 1. A changelog entry in [Architecture Changelog](#14-architecture-changelog). 2. Updated gap-ledger entries if implementation work is required. 3. Updated acceptance criteria or tests when behavior changes. 4. Removal or correction of stale status claims. --- ## Table of Contents 1. [System Overview](#1-system-overview) 2. [Project Structure](#2-project-structure) 3. [Core Architecture](#3-core-architecture) 4. [Planning Pipeline](#4-planning-pipeline) 5. [DAG Execution Engine](#5-dag-execution-engine) 6. [Symbol Indexing](#6-symbol-indexing) 7. [Tool System](#7-tool-system) 8. [Configuration](#8-configuration) 9. [Critical Components](#9-critical-components) 10. [Testing & CI/CD](#10-testing--ci/cd) 11. [Performance](#11-performance) 12. [Implementation Priority](#12-implementation-priority) 13. [Enterprise Quality & Sanity Gates](#13-enterprise-quality--sanity-gates) 14. [Architecture Changelog](#14-architecture-changelog) --- ## 1. System Overview ### What is Rigorix? **Rigorix** is a **deterministic coding CLI** built in Rust. It's a **task graph compiler with execution profiles**, NOT a multi-agent system. ### Core Principles | Principle | Implementation | |-----------|----------------| | **Template-driven** | Workflows in `templates/*.toml`, not dynamic agent generation | | **DAG-based** | Task nodes with dependencies, topological execution | | **Minimal LLM** | LLM = planning tool only (classification, parameter extraction) | | **Bounded autonomy** | Hard mathematical caps on dynamic behavior | | **Bounded retries** | Max 3 retries with exponential backoff + jitter (±25%) | | **Risk-gated** | Safe=auto, Medium=confirm, Dangerous=dry-run | | **Replayable** | Full execution trace with state snapshots (including symbol graph) | | **Pre-validated** | PlanValidator catches errors BEFORE execution | | **Auditable** | Planning decisions tracked and diffable | ### Strategic Differentiators 1. **Deterministic Planning** - Templates define structure, LLM fills parameters 2. **Bounded Execution** - Retry loops with fallbacks (no emergent chaos) 3. **Mathematical Autonomy Caps** - Hard limits on dynamic behavior 4. **Symbol Graph** - Definitions + references with O(1) lookups 5. **Debuggable** - Event bus + replay + explain + plan diff 6. **Risk-Aware** - Tool gating by risk level 7. **Two-Mode** - Default (template) + Advanced (bounded autonomy) 8. **Pre-Validation** - Catches plan errors before execution 9. **Cost Control** - LLM budget tracking and enforcement --- ## 2. Project Structure ### 2.1 Workspace Organization ``` rigorix/ ├── Cargo.toml # Workspace root ├── ARCHITECTURE.md # This document ├── rigorix/ # Main orchestrator crate (library) │ ├── Cargo.toml │ └── src/ │ ├── lib.rs # Library entry point │ ├── main.rs # Binary entry point (thin wrapper) │ ├── core.rs # RiskLevel, core types │ ├── error.rs # Error types (thiserror) │ ├── cancellation"
language: rust
